...
public class DBFReader extends DBFBase implements Closeable {
	 ...
	 */
	public DBFReader(InputStream in,File file) {
		this(in,file,null,false);
	}
	...
	public DBFReader(InputStream in, Boolean showDeletedRows) {
		this(in,null,null, showDeletedRows);
	}
	 ...
	 * @param showDeletedRows can be used to identify records that have been deleted.
	 */
	public DBFReader(InputStream in, File memoFile,Charset charset, boolean showDeletedRows) {
		try {
			...
			if(memoFile!=null){//设置解析莫memo文件
				setMemoFile(memoFile);
			}
	...
	*/
	...
	protected Object getFieldValue(DBFField field) throws IOException {
		...
		case MEMO:
//			this.setMemoFile(new File("D:\\moban\\考生指纹信息.FPT"));
